...
<template>
  <div class="flex h-full min-h-0 bg-ink">
    <aside class="flex w-16 shrink-0 flex-col border-r border-line lg:w-56">
      <div class="flex items-center justify-center gap-3 px-2 py-4 lg:justify-start lg:px-5 lg:py-5">
      ...
      <div class="m-2 rounded-xl border border-line px-2 py-2 text-center lg:m-3 lg:px-3 lg:py-3 lg:text-left" :title="stackLine">
        ...
        <div class="text-[11px] text-muted lg:mt-1">{{ store.runningCount }}/4</div>
      </div>
    </aside>
    ...
    <div class="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      <div v-else-if="store.snap?.message" class="mx-4 mt-3 rounded-lg border border-line px-4 py-3 text-sm text-muted lg:mx-8 lg:mt-4">
        {{ store.snap.message }}
      </div>
